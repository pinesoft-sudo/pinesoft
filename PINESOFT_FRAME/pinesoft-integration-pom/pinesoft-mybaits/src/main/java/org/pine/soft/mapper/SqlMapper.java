...
public class SqlMapper {
	...
	public int selectCount(String sql) {
		...
		Map<String, Object> maps = selectOne(String.format("select count(0) %s from (%s)", COUNT_KEY, sql));
		if (maps.containsKey(COUNT_KEY)) {
			return Integer.parseInt(maps.get(COUNT_KEY).toString());
		}
		return 0;
	}
	...
	private class MSUtils {
		...
		private String newMsId(String sql, SqlCommandType sqlCommandType) {
			...
			lock.lock();
			msIdBuilder.append(".").append(sql.hashCode()).append(System.nanoTime());
			lock.unlock();
